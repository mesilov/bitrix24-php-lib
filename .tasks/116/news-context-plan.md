...
- `src/News/Entity/NewsStatus.php` — enum `draft | published | archived`;
...
- CQRS UseCases: `Create`, `Update`, `Publish`, `Archive`, `RevertToDraft`, `GetList`;
...
2. `NewsStatus` enum с guard'ами переходов: `draft→published` (`publish`), `published→draft` (`revertToDraft`), `*→archived` (`archive`); недопустимые переходы кидают `LogicException`/`InvalidArgumentException`.
...
5. Репозиторий: `save`, `findById`, `findPublished` (пагинированный список).
